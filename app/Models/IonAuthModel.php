...
class IonAuthModel
{
	...
	public function getUserByIdentity(string $identity)
	{
		...
		// Retrieve the user according to this selector
		$user = $this->where($this->config->identity, $identity)->users()->row();
	...
	public function login(string $identity, string $password, bool $remember=false): bool
	{
		...
		$query = $this->db->table($this->tables['users'])
						  ->select($this->identityColumn . ', email, id, password, nickname, fullname, active, last_login')
						  ->where($this->identityColumn, $identity)
						  ->limit(1)
						  ->orderBy('id', 'desc')
						  ->get();
